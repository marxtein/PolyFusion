...
from ..presets import (
    PRESETS as TOKAMAK_PRESETS,
    PARAM_ORDER as TOKAMAK_PARAMS,
...
class ConfigSpec:
    ...
    def solve(self, params: dict) -> dict:
        ...
        bad = [
            ...
            if isinstance(v, complex) or (isinstance(v, float) and not math.isfinite(v))
        ]
    ...
    def validate(self, params: dict) -> list[str]:
        ...
        if "fHe" in self.params:
            ...
            if (
                isinstance(fHe, (int, float))
                and isinstance(fimp, (int, float))
                and fHe >= 0
                and fimp >= 0
                and fHe + fimp >= 1.0
            ):
...
def _frc_cross(p: dict) -> list[str]:
    ...
    if r_s is not None and r_w is not None and r_s >= r_w:
        errors.append(
            f"need r_s < r_w (got r_s={r_s}, r_w={r_w}): "
    ...
    if sep_model is not None and sep_model not in ("superellipse", "ma_xie", "mrr"):
        errors.append(
            "sep_model must be 'superellipse' or 'ma_xie' "
    ...
    if m is not None and f_shape is not None and abs(m / (m + 1.0) - f_shape) > 1e-6:
        errors.append(
            f"inconsistent m and f_shape: m={m} implies "
...
def _dipole_cross(p: dict) -> list[str]:
    ...
    if bool(p.get("ring_model", 0.0)) and r_ring is not None and R_p is not None:
        ...
        if lam_out > 16.0:
            errors.append(
                "finite-ring dipole tables require R_p/r_ring <= 16 "
...
def _stell_cross(p: dict) -> list[str]:
    ...
    if etabar is not None and etabar == 0:
        errors.append(
            "etabar must be != 0 (near-axis shaping required; legacy mode removed)"
        )
...
TOKAMAK = ConfigSpec(
    name="tokamak",
    label="托卡马克 Tokamak",
    params=["use_tauE", "cyclotron_B_nonuniform"]
    + TOKAMAK_PARAMS
    + [
        "imp_name",
        "H_fac",
    ...
    bounds={
        **_COMMON_BOUNDS,
        ...
        "H_fac": (0.0, None),
        "geom_model": (0.0, 2.0),
        "Vp_override": (0.0, None),
        "Sw_override": (0.0, None),
        "f_aux_e": (0.0, 1.0),
    },
    ...
    scan_defaults=dict(
        xkey="Ti0", ykey="ni0", xmin=20, xmax=200, ymin=0.5e20, ymax=4e20
    ),
...
MIRROR = ConfigSpec(
    ...
    scan_defaults=dict(
        xkey="Ti0", ykey="ni0", xmin=5, xmax=200, ymin=0.5e20, ymax=6e20
    ),
...
STELLARATOR = ConfigSpec(
    ...
    positive=["R0", "a", "N_fp", "ni0", "Ti0", "B0", "Zimp", "f_ren"],
    bounds={
        **_COMMON_BOUNDS,
        "delta_h": (0.0, None),
        "N_fp": (1.0, None),
