...
def _base(p):
    ...
    for o in ["delta_h", "iota", "f_ren", "etabar", "f_aux_e", "H_fac", "use_tauE"]:
        if o in p:
            kw[o] = p[o]
    return kw
...
def main():
    ...
    r2 = solve_stellarator(
        **_base(p),
        shape=shape,
        Vp_override=p["Vp_override"],
        Sw_override=p["Sw_override"],
    )
    ok(r2.Vp == p["Vp_override"], f"override: Vp == measured ({r2.Vp})")
    ok(
        abs(r2.Vp_geom - V_int) < 1e-6 * V_int,
