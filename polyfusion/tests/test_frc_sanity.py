...
def main():
    ...
    r = solve_frc(**BASE)
    print(
        f"base: Pfus={r.Pfus:.3f}MW Q={r.Qfus:.3f} beta={r.beta:.3f} "
    ...
    allok &= _ok(
        all(
            ...
            for v in [r.Pfus, r.tau_E, r.Vp, r.ne0, r.ni0, r.beta]
        ),
    ...
    allok &= _ok(
        hi.ni0 > r.ni0 and hi.Pfus > r.Pfus,
    ...
    allok &= _ok(
        big.tau_E > self_base.tau_E and big.Qfus > self_base.Qfus,
    ...
    allok &= _ok(
        near.beta < r.beta, f"x_s up -> beta down ({r.beta:.3f}->{near.beta:.3f})"
    )
