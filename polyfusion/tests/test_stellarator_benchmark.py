...
from polyfusion.configs.stellarator import (
    axis_length,  # noqa: E402
    section_outlines,
...
def main():
    ...
    ok(
        abs(st.Vp - Vp_exact) / Vp_exact < 3e-3,
    ...
    ok(
        0.8 < H < 1.5,
        f"W7-X record-shot H_ISS04 = {H:.2f} with measured iota=0.88 (lit. 1-1.4)",
    )
    ...
    na = solve_stellarator(
        R0=18.0, N_fp=3, delta_h=0.045 * 18.0, etabar=0.9 / 18.0, **common
    )
    ok(
        abs(na.iota_geom - 0.418306910215178) < 1e-6,
        f"near-axis NAE-QA: iota = {na.iota_geom:.9f} (published 0.418306910)",
    )
    ok(
        abs(na.elong_max - 2.41373705531443) / 2.41373705531443 < 2e-3,
    ...
    _bfn, _nfp = _nearaxis_boundary_fn(
        R0=18.0, A=10.0, N_fp=3, delta_h=0.045 * 18.0, etabar=0.9 / 18.0
    )
    V_int, _, _ = boundary_metrics(_bfn, _nfp, 0.1)
    ok(
        abs(na.Vp - V_int) / V_int < 1e-9,
    ...
    gm = stellarator_geometry_metrics(
        R0=18.0,
        A=10.0,
        N_fp=3,
        rc=[18.0, 0.045 * 18.0],
    ...
    na2 = solve_stellarator(
        R0=18.0, N_fp=3, delta_h=0.08 * 18.0, etabar=0.9 / 18.0, **common
    )
    ok(
        abs(na2.iota_geom - na.iota_geom) > 0.05,
    ...
    nqh = solve_stellarator(
        R0=10.0, N_fp=4, delta_h=2.65, etabar=-2.25 / 10.0, **{**common, "A": 8.0}
    )
    ok(
        abs(nqh.helicity) == 1 and nqh.iota_geom > 0.5,
    ...
    for s in nae["sections"]:
        ...
        ok(
            all(np.isfinite(s["R"])) and all(np.isfinite(s["Z"])),
            f"near-axis section {s['label']}: finite outline",
        )
    # the 0-D power-account section area stays first-order pi*a^2 (unchanged)
    gm = stellarator_geometry_metrics(
        R0=18.0, a=1.8, N_fp=3, rc=[18.0, 0.81], zs=[0.0, -0.81], etabar=0.05
    )
    ok(
        abs(gm["A_flux"] - math.pi * a**2) < 1e-9,
