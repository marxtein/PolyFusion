...
def main():
    ...
    na = solve_near_axis(
        [1.3, 0.3, 0.01, -0.001], [0, 0.4, -0.02, -0.003], nfp=5, etabar=1.0, nphi=15
    )
    ok(
        np.allclose(na.curvature, GOLD_CURV, rtol=1e-11, atol=1e-11),
        f"curvature matches pyQSC golden (max dev {np.max(np.abs(na.curvature - GOLD_CURV)):.2e})",
    )
    ok(
        np.allclose(na.torsion, GOLD_TORS, rtol=1e-11, atol=1e-11),
    ...
    for name, rc, zs, nfp, etab, iota_ref, elong_ref, hel_ref in R1_CASES:
        ...
        ok(
            abs(na.iota - iota_ref) / abs(iota_ref) < 1e-8,
            f"{name}: iota = {na.iota:.12f} (ref {iota_ref})",
        )
        ok(
            abs(na.max_elongation - elong_ref) / elong_ref < 2e-3,
    ...
    ok(
        abs(a.iota - b.iota) < 1e-10,
        f"iota scale-invariant ({a.iota:.9f} vs {b.iota:.9f})",
    )
    ok(
        abs(b.axis_length - s * a.axis_length) / (s * a.axis_length) < 1e-12,
    ...
    ok(
        abs(d.iota - a.iota) < 1e-9,
        f"iota converged already at nphi=31 (dev {abs(d.iota - a.iota):.1e})",
    )
    ...
    ok(
        abs(e.iota) < 1e-10,
        f"planar circular axis: iota = {e.iota:.2e} (axisymmetric -> 0)",
    )
