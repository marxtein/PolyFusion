...
def main():
    ...
    bfn, nfp = _nearaxis_boundary_fn(R0=18.0, a=1.8, N_fp=3, delta_h=0.9, etabar=0.05)
    ...
    ok(
        abs(r.Vp - V_int) < 1e-6 * V_int,
        f"concept Vp == boundary integral ({r.Vp:.3f} == {V_int:.3f})",
    )
    ok(
        abs(r.Sw - S_int) < 1e-6 * S_int,
    ...
    disp = section_outlines(
        **{k: cfg[k] for k in ("R0", "a", "N_fp", "delta_h", "etabar", "g")}
    )
    Rd = disp["sections"][0]["R"]
    Zd = disp["sections"][0]["Z"]
    cf_disp = _concave_frac(Rd, Zd)
    ok(cf_int < 0.5, f"integral boundary is simple (concave {cf_int:.2f} < 0.5)")
    ok(
        abs(cf_int - cf_disp) < 0.1,
