...
def main():
    ...
    for K in (0.3, 1.0, 1.9):
        ...
        tK = math.tanh(K)
        ok(
            abs(num_G1 - tK / K) < 1e-6,
            f"K={K}: <sech2> analytic vs numeric ({num_G1:.6f})",
        )
        ok(
            abs(num_G2 - (tK - tK**3 / 3) / K) < 1e-6,
            f"K={K}: <sech4> analytic vs numeric ({num_G2:.6f})",
        )
        ok(
            abs(num_GB - math.log(math.cosh(K)) / K) < 1e-6,
            f"K={K}: <|tanh|> analytic vs numeric ({num_GB:.6f})",
        )
    ...
    num_flux = (
        ...
        * r_s**2
        * B_e
        * float(np.trapezoid(np.abs(np.tanh(K * (2 * y - 1))), y))
    )
    ana_flux = math.pi * r_s**2 * B_e * math.log(math.cosh(K)) / (2 * K)
    ok(
        abs(num_flux - ana_flux) / ana_flux < 1e-5,
    ...
    ok(
        legacy.geom_weighted == 0.0 and weighted.geom_weighted == 1.0,
        "legacy and geometry-weighted FRC modes are both selectable",
    )
    ok(
        abs(weighted.f_shape_calc - 0.85) < 1e-7 and weighted.p_shape > 2.0,
        "weighted mode exposes the superellipse p_shape used by the power account",
    )
    ok(
        abs(legacy.Vp - weighted.Vp) / legacy.Vp < 1e-12
        and legacy.Pfus != weighted.Pfus,
    ...
    ok(
        c.get("shape", {}).get("type") == "frc",
        "FRC run_case returns backend shape data",
    )
    ok(
        abs(c["shape"]["p_shape"] - c["outputs"]["p_shape"]) < 1e-9,
    ...
    r = solve_frc(
        r_s=0.2, l_s=2.0, r_w=0.45, B_e=0.5, Ti=0.2, Te=0.15, icase=1, use_tauE=0.0
    )
    ok(
        5e-5 < r.tau_E < 5e-3,
        f"module LSX-like tau_E = {r.tau_E * 1e3:.2f} ms (same order)",
    )
