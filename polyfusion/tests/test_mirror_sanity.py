...
def main():
    ...
    r = solve_mirror(**BASE)
    print(
        f"base: Pfus={r.Pfus:.3f}MW Q={r.Qfus:.3f} tau_m={r.tau_m:.3e}s "
    ...
    allok &= _ok(
        all(
            ...
            for v in [r.Pfus, r.tau_m, r.Vp, r.ne0, r.beta, r.phi_i, r.phi_e]
        ),
    ...
    allok &= _ok(
        r.tau_m <= r.tau_rho and r.tau_m <= (r.tau_Past + r.tau_gd) + 1e-30,
        "tau_m <= each channel",
    )
    ...
    allok &= _ok(
        r.B0 < BASE["B_vac"] and r.R_mc > BASE["R_mirror"],
    ...
    allok &= _ok(
        math.isfinite(pb.Pfus) and pb.Pfus > 0, f"p-B11 runs (Pfus={pb.Pfus:.3f}MW)"
    )
    ...
    allok &= _ok(
        okf,
        f"flat limit reproduces v1 (Pfus={flat.Pfus:.3f} Q={flat.Qfus:.3f} tau={flat.tau_m:.4f} beta={flat.beta:.3f})",
    )
