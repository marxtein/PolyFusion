...
from polyfusion.twotemp import (
    critical_energy,
    ion_deposition_fraction,  # noqa: E402
    equilibration_time,
    p_ei_exchange,
)
...
def main():
    ...
    ok(
        0.10 < f_alpha < 0.35,
        f"D-T alpha ion fraction = {f_alpha:.3f} (~0.2: mostly electrons!)",
    )
    fr = [ion_deposition_fraction(E, Ec) for E in (50, 200, 1000, 3500)]
    ok(
        all(a > b for a, b in zip(fr, fr[1:])),
        "Stix fraction monotone decreasing in E0",
    )
    ...
    for sp in ("C", "N", "Ne", "Ar", "Kr", "Xe", "W"):
        ...
        ok(
            np.all(net >= 0) and np.all(np.isfinite(net)),
    ...
    ok(
        abs(K[0] - math.pi / 2) < 1e-12 and abs(E[0] - math.pi / 2) < 1e-12,
        "K(0) = E(0) = pi/2",
    )
    ok(
        abs(K[1] - 1.8540746773) < 1e-9 and abs(E[1] - 1.3506438810) < 1e-9,
    ...
    ok(
        o["tau_eta"] > 0 and math.isfinite(o["tauN_o_taueta"]),
