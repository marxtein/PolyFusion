...
def main() -> int:
    ...
    allok &= ok("A" not in meta["params"], "stellarator UI/API no longer exposes aspect ratio A")

    spec = get("stellarator")
    w7x = dict(spec.presets["W7-X"])
    allok &= ok("a" in w7x and "A" not in w7x,
                "packaged W7-X preset stores a, not legacy A")
