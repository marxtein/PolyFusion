...
def main():
    ...
    print(
        f"base: Vp={b.Vp:.2f} Sw={b.Sw:.2f} Pfus={b.Pfus:.2f} Pheat={b.Pheat:.2f} "
    ...
    allok &= _ok(
        abs(so.Pwall - pwall_expect) < 1e-9 * max(abs(pwall_expect), 1.0),
    ...
    pm = solve_stellarator(
        **{**BASE, "delta_h": 0.0, "etabar": 0.04, "shape": circle, "iota": 0.40}
    )
    allok &= _ok(
        pm.iota == 0.40 and math.isfinite(pm.Pfus) and pm.Pfus > 0,
    ...
    allok &= _ok(
        abs(ca.iota_geom - b.iota_geom) > 1e-3,
