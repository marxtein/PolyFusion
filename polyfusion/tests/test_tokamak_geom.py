...
def test_shape_outlines_closed_and_wall_outside():
    out = tg.tokamak_shape_outlines(
        R0=6.2, A=3.1, kappa=1.7, delta=0.33, g=0.1, geom_model=2, eq=None
    )
    lcfs = out["lcfs"]
    wall = out["wall"]
    ...
    for name, preset in spec.presets.items():
        for gm in (0, 1, 2):
            ...
            assert out["valid"] == 1.0, (
                f"{name} invalid under geom_model={gm}: {out.get('invalid_fields')}"
            )
            assert out["Vp"] > 0
...
def test_cf_low_aspect_no_log_domain_warning():
    ...
    with warnings.catch_warnings():
        ...
        R, Z, shaf = tg.cf_boundary(R0=0.7, a=0.4375, kappa=2.5, delta=0.5, n_theta=600)
...
def test_shape_outlines_eq_has_flux_surfaces():
    ...
    eq = eqdsk.equilibrium_geometry(eqdsk.parse_geqdsk(open(fx).read()))
    out = tg.tokamak_shape_outlines(
        R0=6.2, A=3.1, kappa=1.7, delta=0.33, g=0.05, geom_model=2, eq=eq
    )
...
def test_miller_flux_surfaces_nested_and_rounder_inward():
    ...
    def _elong(s):
        return (max(s["Z"]) - min(s["Z"])) / (max(s["R"]) - min(s["R"]))

    assert _elong(fs[0]) < _elong(fs[-1])  # inner rounder than outer
