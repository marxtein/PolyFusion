...
def test_volume_weight_self_similar_reduces_to_2x():
    x = np.linspace(0.0, 1.0, 101)
    dx = x[1] - x[0]
    w = tg.volume_weight(6.35, 3.43, 1.0, 0.0, x)        # circle => self-similar
    assert float(np.sum(w * dx)) == pytest.approx(1.0, rel=1e-6)
    for p in (1.0, 1.5, 2.0):
        G = float(np.sum((1 - x**2) ** p * w * dx))
        assert G == pytest.approx(1.0 / (1.0 + p), rel=0.01)


def test_volume_weight_shaped_lowers_form_factor():
    x = np.linspace(0.0, 1.0, 101)
    dx = x[1] - x[0]
    w = tg.volume_weight(6.2, 3.12, 1.88, 0.42, x)       # ITER-ish shaped
    G2 = float(np.sum((1 - x**2) ** 1.0 * w * dx))
    assert 0.42 < G2 < 0.48                               # ~10% below self-similar 0.5


def test_funsc_geom_weighted_off_identical_on_lowers_power():
    from polyfusion.tokamak import funsc
    base = dict(R0=6.35, A=3.43, kappa=1.86, delta=0.5, Sn=0.5, ST=1.0,
                ni0=6.81e19, Ti0=25, fT=1.0, fsig=1.0, f1=0.5, BT0=5.18,
                Ip=9.2, tauE=2.0, fHe=0.04, fimp=0.01, Zimp=10, Rw=0.7,
                g=0.05, icase=1)
    r0 = funsc(**base)
    r0b = funsc(**base, geom_weighted=0)
    assert r0.Pfus == r0b.Pfus                       # default == explicit off (bit-identical)
    assert r0.Eth == r0b.Eth and r0.Pbrem == r0b.Pbrem
    rw = funsc(**base, geom_weighted=1)
    assert rw.Pfus < r0.Pfus                         # geometry weighting lowers fusion power
    assert rw.Eth < r0.Eth
    # Pfus drops ~17% (reactivity is core-peaked, amplifying the core de-weighting);
    # Eth drops ~14% (pure (1-x^2)^(Sn+ST) form factor).
    assert 0.78 < rw.Pfus / r0.Pfus < 0.90
    assert 0.82 < rw.Eth / r0.Eth < 0.92


def test_config_spec_geom_weighted_lowers_power():
    from polyfusion.configs.base import get
    spec = get("tokamak")
    assert "geom_weighted" in spec.params
    p = spec.presets["ITER"]
    off = spec.solve({**p, "geom_weighted": 0})
    on = spec.solve({**p, "geom_weighted": 1})
    assert on["valid"] == 1.0
    assert on["Pfus"] < off["Pfus"]
    assert on["Eth"] < off["Eth"]
