...
def main():
    ...
    ok(
        abs(_rr_flux_norm(xo, K) - psi_o) < 1e-12,
        f"psi_tilde(1/sqrt2) = -ln cosh K = {psi_o:.6f} (O point, the minimum)",
    )
    ok(
        _rr_flux_norm(0.5, K) < 0 and _rr_flux_norm(0.9, K) < 0,
    ...
    for frac in (0.2, 0.5, 0.8):
        ...
        ok(
            0.0 < x_in < xo < x_out < 1.0,
            f"frac={frac}: 0 < x_in={x_in:.4f} < 1/sqrt2 < x_out={x_out:.4f} < 1",
        )
        ok(
            abs(_rr_flux_norm(x_in, K) - psi_star) < 1e-9
    ...
    ok(
        abs(xim - xo) < 1e-3 and abs(xom - xo) < 1e-3,
    ...
    for s in surfs:
        ...
        r = np.asarray(s["r"])
        ok(
            abs(r[0] - r[-1]) < 1e-9 and abs(z[0] - z[-1]) < 1e-9,
    ...
    for mode in ("superellipse", "ma_xie"):
        d = frc_shape_outlines(r_s=0.5, l_s=5.0, r_w=0.7, f_shape=0.85, sep_model=mode)
        ok(
            isinstance(d.get("surfaces"), list) and len(d["surfaces"]) > 0,
        ...
        ok(
            op is not None
            and abs(op["r"][0] - 0.5 / math.sqrt(2)) < 1e-9
            and abs(op["z"][0]) < 1e-12,
            f"{mode}: O-point at (z=0, r=r_s/sqrt2)",
        )
        ok(
            xp is not None and abs(xp["z"][0] - 2.5) < 1e-9 and abs(xp["r"][0]) < 1e-12,
        ...
        sz = np.array(d["separatrix"]["z"])
        sr = np.array(d["separatrix"]["r"])
        up = sr >= -1e-12
        zz = sz[up]
        rr = sr[up]
        o = np.argsort(zz)
        ...
        for s in d["surfaces"]:
            z = np.array(s["z"])
            r = np.abs(np.array(s["r"]))
            worst = max(worst, float(np.max(r - np.interp(z, zz, rr))))
        ok(
            worst <= 1e-6,
            f"{mode}: surfaces inside separatrix envelope (max overshoot {worst:.2e})",
        )

        # ---- open SOL field lines OUTSIDE the separatrix (audit docs/42 P2) ----
        ol = d.get("open_lines")
        ok(
            isinstance(ol, list) and len(ol) > 0,
        ...
        ok(
            spans,
            f"{mode}: open lines extend past the separatrix ends (open, not closed)",
        )
        ok(
            all((min(s["r"]) > -1e-9) or (max(s["r"]) < 1e-9) for s in ol),
