...
def sigma_dt(E):
    ...
    Eh = E[hi]
    S[hi] = -1.4714e6 / (
        1.0 - 8.4127e-3 * Eh + 4.7983e-6 * Eh**2 - 1.0748e-9 * Eh**3 + 8.85184e-14 * Eh**4
    )
...
def sigma_dd_total(E):
    ...
    ok = (E > 0) & (E <= 4900)
    Eo = E[ok]
    Sdd1 = 5.5576e4 + 2.1054e2 * Eo - 3.2638e-2 * Eo**2 + 1.4987e-6 * Eo**3 + 1.8181e-10 * Eo**4
    Sdd2 = 5.3701e4 + 3.3027e2 * Eo - 1.2706e-1 * Eo**2 + 2.9327e-5 * Eo**3 - 2.5151e-9 * Eo**4
...
def sigma_dhe(E):
    ...
    Eh = E[hi]
    S[hi] = -8.3993e5 / (
        1.0 - 2.6830e-3 * Eh + 1.1633e-6 * Eh**2 - 2.1332e-10 * Eh**3 + 1.4250e-14 * Eh**4
    )
    return S / (E * np.exp(BG / np.sqrt(E))) * 1e-31
...
def sigma_pb(E):
    ...
    seg3 = E > 642
    E1 = E[seg1]
    S[seg1] = 1.97e5 + 0.24e3 * E1 + 2.31e-1 * E1**2 + 1.82e7 / ((E1 - 148) ** 2 + 2.35**2)
    E2 = E[seg2] / 100.0 - 4.0
