...
def resolve_cyclotron_power(
    formula_power: float, electron_energy: float, use_tauC: float, tauC: float | None
) -> tuple[float, float]:
    ...
    return (
        formula_power,
        electron_energy / formula_power if formula_power > 0 else 1e300,
    )
...
def tokamak_B25_factor(R0: float, a: float, kappa: float, delta: float) -> float:
    ...
    return float(
        np.sum(quad * volume_weight * (R0 / R) ** 2.5) / np.sum(quad * volume_weight)
    )
