...
\begin{abstract}
本报告把 PolyFusion 0-D 程序里\textbf{整个仿星器位形}从头讲透：它\textbf{算什么物理}（0-D 功率平衡）、\textbf{怎么描述几何}（近轴展开 + 真机傅里叶边界）、\textbf{几何如何进功率账}（哪个量、什么幂次）、\textbf{怎么画截面}（统一嵌套磁面 + 相位滑块）、\textbf{内置哪些位形}（9 个预设），以及 \textbf{POPCON 扫描怎么算、怎么被加速}。目标是让懂等离子体物理、但没读过这套代码的人，能完整地"听懂"每一步在做什么、为什么这么做、近似在哪里。核心观念贯穿全文：几何有\textbf{两个互相独立又一致}的角色——给人看的\emph{截面视图}，和给功率公式做积分权重的\emph{几何标量}$(a,\Vp,\Sw,\Lax,\iotabar)$；二者通过同一个截面面积锚 $\Aflux=\pi a^2$ 绑定。
\end{abstract}
...
\paragraph{本报告范围。}
只讲\textbf{仿星器位形}：第 \ref{sec:power} 节物理内核，第 \ref{sec:nae}--\ref{sec:integral} 节几何（近轴、真机边界、体积积分），第 \ref{sec:account} 节几何进功率，第 \ref{sec:view} 节截面可视化，第 \ref{sec:presets} 节 9 个预设，第 \ref{sec:popcon} 节 POPCON 与性能，第 \ref{sec:consistency} 节一致性与局限。代码在 \texttt{polyfusion/configs/stellarator.py}、\texttt{polyfusion/nearaxis.py}、\texttt{polyfusion/presets/stellarator.json}。
...
\item \textbf{回旋辐射} $\Pcycl\propto B_0^{2.5}\,T_\mathrm{eff}^{2.5}\,n_\mathrm{eff}^{0.5}(1-R_w)^{0.5}\,a^{-0.5}\,\Vp$，$R_w$ 是壁反射率。注意它\textbf{显含小半径} $a^{-1/2}$——回旋辐射对装置尺寸敏感。
...
\begin{equation}
\boxed{\;\tau_\mathrm{ISS04}=0.134\,f_\mathrm{ren}\,a^{2.28}R_0^{0.64}P_L^{-0.61}
...
\[
n_\mathrm{Sudo}=0.25\sqrt{P_L B_0/(a^2 R_0)}\times10^{20}.
\]
两个自洽回路：$\tau_E=0$ 时从 ISS04 预测求解 $\tau_E$（令 $H_\mathrm{ISS04}=H_\mathrm{fac}$）；$f_T=0$ 时从电子通道平衡自洽解 $T_e/T_i$。

\paragraph{小结。}功率账只认一组\textbf{标量}：$\Vp$（最强，进 $\Pfus,\Pbrem,\Pcycl,\Eth$）、$\Sw$（只进壁负荷 $\Pwall=(\Pfus+\Pheat)/\Sw$）、$a,R_0,\iotabar$（进 ISS04/Sudo）。\textbf{截面形状本身不进功率}——它纯粹是几何标量的来源和给人看的图。下面三节就讲这些标量从哪来。
...
\end{equation}
随 $\etabar$ 减小而增大（$\sim(\kappa/\etabar)^2$）。\textbf{重要}：一阶面积 $\propto X_{1c}Y_{1s}=1$（面积守恒）——拉长比变、面积不变，这就是后面 $\Aflux=\pi a^2$ 锚的根据：\textbf{塑形不改体积}。
...
\paragraph{做法。}
真机预设携带一个显式 \texttt{shape}：取\textbf{公开平衡}（DESC \texttt{desc/examples}：W7-X、HSX、ESTELL；经典 $l{=}2$ heliotron 作 LHD）的边界 Fourier 系数，\textbf{截断} $|m|,|n|\le2$、\textbf{归一化}（减 $R_{00}$、除原生小半径），按 DESC 双 Fourier 乘积基求值，再\textbf{重缩放}到本预设的 $R_0,a=R_0/A$：
\begin{equation}
...
\]
这复现了 W7-X bean、LHD 旋转椭圆、HSX/CFQS 形态。\textbf{它是 cartoon}：$|m|,|n|\le2$ 截断、非完整平衡重建；纯供\emph{显示}和\emph{几何积分估算}，\textbf{不进功率账}（真机功率用实测 $\iotabar,\Vp,\Sw$ 覆写）。

%======================================================================
\section{体积与壁面：精确边界积分}\label{sec:integral}

无论概念堆还是真机，\textbf{功率账要的体积 $\Vp$ 和壁面 $\Sw$ 都来自对边界的精确环向积分}，而且积分的就是\textbf{前端画出来的那一条边界}（前后端一致）。
...
\paragraph{面积锚。}
$\Aflux=\pi a^2$ 仍是\textbf{功率密度}的一阶通量守恒锚（一阶椭圆面积恒为 $\pi a^2$，拉长比不改它）。$\Lax$ 是近轴磁轴弧长。真机另带实测 $\Vp^{*}/\Sw^{*}$ 覆写。
...
ISS04 约束 & $\tau\propto a^{2.28}R_0^{0.64}P_L^{-0.61}\bar n^{0.54}B_0^{0.84}\iotabar^{0.41}$ & $a,R_0,\iotabar$ \\
Sudo 密度限 & $n_\mathrm{Sudo}=0.25\sqrt{P_LB_0/(a^2R_0)}\,10^{20}$ & $a,R_0$ \\
\bottomrule
\end{tabular}
\end{center}
...
\item \textbf{$\Sw$ 只进壁负荷}。真机要 $\Sw^{*}$ 覆写，因近轴 $\Sw$ 在极端拉长下失真（如 HSX 近轴 $e_\mathrm{max}\!\sim\!384$）。
\item \textbf{$\iotabar$ 仅以 $0.41$ 次幂进 ISS04}。真机用实测 $\iotabar$；概念堆用近轴自洽解。
\item \textbf{形状不进功率}：bean/D 形/旋转椭圆只决定上述标量和图，功率公式只看标量。
\end{itemize}
...
\paragraph{相位扫描。}
仿星器截面随 $\varphi$ 变，UI 提供\textbf{相位开关}（$\varphi=0/T4/T2$ 三个特征切面）与\textbf{连续相位滑块}（真机与概念堆\textbf{统一}支持——两条路径都输出一周期 $24$ 帧）。滑块平滑的关键：把坐标视窗\textbf{固定到全帧并集}，相位扫描时截面\emph{原地形变}，而不是每帧重缩放（否则视觉抖动）。

\paragraph{壁层与高级输入。}
壁层 $=$ 边界沿外法向外扩 $g$，保证 磁面 $\subset$ 边界 $\subset$ 壁。高级输入（自定义轴 $rc/zs$、实测 $\Vp^{*}/\Sw^{*}$）折叠在\textbf{几何参数组末尾}就地展开，普通模式只露 $R_0/A/\nfp/\delta_h/\etabar/g$。
...
\end{tabular}
\end{center}
\footnotesize $^{*}$ 真机的 $\iotabar,\Vp,\Sw$ 为\textbf{实测覆写}，进功率账；其近轴诊断仅供参考。\normalsize
...
\item[真机（边界谐波）] W7-X / LHD / HSX / CFQS。单谐波近轴无法表示，携带 $|m|,|n|\le2$ 归一化边界谐波\emph{仅供显示}，功率账用实测 $\iotabar/\Vp^{*}/\Sw^{*}$。因实验装置温度低（$T_i\!\sim\!0.5$--$2$ keV），其 $\Pfus\approx0$（这是\textbf{正确}的——它们演示几何/约束，非反应堆）。
\end{description}
...
\item \textbf{几何是扫描不变量、却每点重算}。POPCON 扫的 $T_{i0}/n_{i0}/B_0$ 都\textbf{不改几何}（几何只依赖 $R_0,A,\nfp,\delta_h,\etabar,rc,zs,g$），但旧代码在 $441$ 个点 + 每次 $\tau_E/T_e$ 自洽递归都重算近轴解 + 边界积分。利用这个\textbf{场周期不变性}，把几何按几何参数 \emph{memoize}（\texttt{\_stellarator\_geometry}）：整张图只算\textbf{一次}几何；若扫描键\emph{是}几何量（$R_0/\etabar/\dots$）则键变、正确重算，永不 stale。
...
\item 真机 $\Vp/\Sw/\iotabar$ 覆写为公开估计，需对照权威文献核实；$\Sw$ 部分为近似（见代码 \texttt{\_notes}）。
