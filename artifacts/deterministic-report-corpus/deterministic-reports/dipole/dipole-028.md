...
- 当前运行点：`valid=1`、`ignited=0`；`Pfus=4.594`、`Qfus=0.6068`、`Pheat=7.571`、`Pwall=0.00968`。
- 当前点是数值有效的 0-D 解。`Qfus` 低于 1，当前点表现为外部加热主导。
- POPCON 扫描为 `Ti0 × n0`，`n_invalid=0`；当前准则下未找到最佳区。
- `use_tauE=1`，结果依赖固定约束时间假设。

# 2. 关键指标解读
- 功率与损失：`Pfus=4.594`、`Qfus=0.6068`、`Pheat=7.571`、`Pwall=0.00968`、`Ptrans=7.379`、`Pbrem=3.211`、`Pcycl=0.03404`、`P_line=0`、`Pn=1.541`。
- 约束、成分与几何：`tau_E=5`、`Zeff=1`、`Vp=1890`、`Sw=1257`。
- 偶极场 Dipole 专属指标：`beta_in=0.7453`、`beta_out=0.1712`、`tauC_eff=329.9`、`cyclotron_model=equatorial_shell_proxy`。
- 当前点对默认 best 准则的逐项核对：`Pfus=4.594` 满足 `>=1`；`Qfus=0.6068` 不满足 `>=1`；`beta_in=0.7453` 满足 `<=1`。
