...
- 当前运行点：`valid=1`、`ignited=0`；`Pfus=17.09`、`Qfus=1.55`、`Pheat=11.03`、`Pwall=0.02238`。
- 当前点是数值有效的 0-D 解。`Qfus` 已达到 1 以上，但仍依赖外部加热且不等同于点火。
- POPCON 扫描为 `Ti0 × n0`，`n_invalid=0`；当前准则下未找到最佳区。
- `use_tauE=1`，结果依赖固定约束时间假设。

# 2. 关键指标解读
- 功率与损失：`Pfus=17.09`、`Qfus=1.55`、`Pheat=11.03`、`Pwall=0.02238`、`Ptrans=14.19`、`Pbrem=8.155`、`Pcycl=0.04297`、`P_line=0`、`Pn=5.734`。
- 约束、成分与几何：`tau_E=5`、`Zeff=1`、`Vp=1890`、`Sw=1257`。
- 偶极场 Dipole 专属指标：`beta_in=1.433`、`beta_out=0.3292`、`tauC_eff=416.4`、`cyclotron_model=equatorial_shell_proxy`。
- 当前点对默认 best 准则的逐项核对：`Pfus=17.09` 满足 `>=1`；`Qfus=1.55` 满足 `>=1`；`beta_in=1.433` 不满足 `<=1`。
