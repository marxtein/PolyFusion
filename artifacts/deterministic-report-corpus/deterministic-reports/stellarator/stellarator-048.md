...
- 当前运行点：`valid=1`、`ignited=1`；`Pfus=1.383e+04`、`Qfus=1000`、`Pheat=-272.8`、`Pwall=8.511`。
...
# 2. 关键指标解读
- 功率与损失：`Pfus=1.383e+04`、`Qfus=1000`、`Pheat=-272.8`、`Pwall=8.511`、`Ptrans=2118`、`Pbrem=343.9`、`Pcycl=32.69`、`P_line=0`、`Pn=1.107e+04`。
- 约束、成分与几何：`tauE_used=1`、`Zeff=1.867`、`Vp=1149`、`Sw=1593`。
- 仿星器 Stellarator 专属指标：`H_ISS04=1.732`、`betaT=0.1235`、`nbar_o_Sudo=0.7276`、`iota=0.8221`、`aspect_vol=10.01`。
- 当前点对默认 best 准则的逐项核对：`Pfus=1.383e+04` 满足 `>=10`；`Qfus=1000` 满足 `>=1`；`nbar_o_Sudo=0.7276` 满足 `<=1`；`betaT=0.1235` 不满足 `<=0.05`；`H_ISS04=1.732` 不满足 `<=1.5`。
