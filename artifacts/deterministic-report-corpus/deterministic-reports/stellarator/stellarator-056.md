...
- 当前运行点：`valid=1`、`ignited=1`；`Pfus=1.639e+04`、`Qfus=1000`、`Pheat=-384.3`、`Pwall=10.04`。
...
# 2. 关键指标解读
- 功率与损失：`Pfus=1.639e+04`、`Qfus=1000`、`Pheat=-384.3`、`Pwall=10.04`、`Ptrans=2470`、`Pbrem=373.8`、`Pcycl=48.78`、`P_line=0`、`Pn=1.311e+04`。
- 约束、成分与几何：`tauE_used=1`、`Zeff=1.867`、`Vp=1149`、`Sw=1593`。
- 仿星器 Stellarator 专属指标：`H_ISS04=1.896`、`betaT=0.1441`、`nbar_o_Sudo=0.6756`、`iota=0.8221`、`aspect_vol=10.01`。
- 当前点对默认 best 准则的逐项核对：`Pfus=1.639e+04` 满足 `>=10`；`Qfus=1000` 满足 `>=1`；`nbar_o_Sudo=0.6756` 满足 `<=1`；`betaT=0.1441` 不满足 `<=0.05`；`H_ISS04=1.896` 不满足 `<=1.5`。
