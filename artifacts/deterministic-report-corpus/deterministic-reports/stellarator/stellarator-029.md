...
- 当前运行点：`valid=1`、`ignited=0`；`Pfus=3148`、`Qfus=7.827`、`Pheat=402.2`、`Pwall=2.228`。
...
# 2. 关键指标解读
- 功率与损失：`Pfus=3148`、`Qfus=7.827`、`Pheat=402.2`、`Pwall=2.228`、`Ptrans=907.5`、`Pbrem=115.1`、`Pcycl=9.227`、`P_line=0`、`Pn=2519`。
- 约束、成分与几何：`tauE_used=1`、`Zeff=1.867`、`Vp=1149`、`Sw=1593`。
- 仿星器 Stellarator 专属指标：`H_ISS04=1.284`、`betaT=0.05293`、`nbar_o_Sudo=0.7272`、`iota=0.8221`、`aspect_vol=10.01`。
- 当前点对默认 best 准则的逐项核对：`Pfus=3148` 满足 `>=10`；`Qfus=7.827` 满足 `>=1`；`nbar_o_Sudo=0.7272` 满足 `<=1`；`betaT=0.05293` 不满足 `<=0.05`；`H_ISS04=1.284` 满足 `<=1.5`。

# 3. 风险与不确定性
- `valid` 只描述单点数值有效性，不能替代 POPCON 工作窗证据。
- 扫描缺少完整 `x/y` 坐标证据，不能给出最佳区实际边界或敏感性方向。
- 当前点未满足默认 best 准则中的：`betaT`。
