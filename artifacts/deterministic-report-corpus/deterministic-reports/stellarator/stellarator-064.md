...
- 当前运行点：`valid=1`、`ignited=1`；`Pfus=1.854e+04`、`Qfus=1000`、`Pheat=-413.1`、`Pwall=11.38`。
...
# 2. 关键指标解读
- 功率与损失：`Pfus=1.854e+04`、`Qfus=1000`、`Pheat=-413.1`、`Pwall=11.38`、`Ptrans=2823`、`Pbrem=402.5`、`Pcycl=69.12`、`P_line=0`、`Pn=1.483e+04`。
- 约束、成分与几何：`tauE_used=1`、`Zeff=1.867`、`Vp=1149`、`Sw=1593`。
- 仿星器 Stellarator 专属指标：`H_ISS04=2.053`、`betaT=0.1647`、`nbar_o_Sudo=0.6331`、`iota=0.8221`、`aspect_vol=10.01`。
- 当前点对默认 best 准则的逐项核对：`Pfus=1.854e+04` 满足 `>=10`；`Qfus=1000` 满足 `>=1`；`nbar_o_Sudo=0.6331` 满足 `<=1`；`betaT=0.1647` 不满足 `<=0.05`；`H_ISS04=2.053` 不满足 `<=1.5`。
