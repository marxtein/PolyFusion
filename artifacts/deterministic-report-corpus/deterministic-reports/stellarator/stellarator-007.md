...
- 当前运行点：`valid=1`、`ignited=0`；`Pfus=120.8`、`Qfus=0.3047`、`Pheat=396.3`、`Pwall=0.3245`。
...
# 2. 关键指标解读
- 功率与损失：`Pfus=120.8`、`Qfus=0.3047`、`Pheat=396.3`、`Pwall=0.3245`、`Ptrans=310.9`、`Pbrem=109.3`、`Pcycl=0.3219`、`P_line=0`、`Pn=96.61`。
- 约束、成分与几何：`tauE_used=1`、`Zeff=1.867`、`Vp=1149`、`Sw=1593`。
- 仿星器 Stellarator 专属指标：`H_ISS04=0.6262`、`betaT=0.01813`、`nbar_o_Sudo=1.561`、`iota=0.8221`、`aspect_vol=10.01`。
- 当前点对默认 best 准则的逐项核对：`Pfus=120.8` 满足 `>=10`；`Qfus=0.3047` 不满足 `>=1`；`nbar_o_Sudo=1.561` 不满足 `<=1`；`betaT=0.01813` 满足 `<=0.05`；`H_ISS04=0.6262` 满足 `<=1.5`。

# 3. 风险与不确定性
- `valid` 只描述单点数值有效性，不能替代 POPCON 工作窗证据。
- 扫描缺少完整 `x/y` 坐标证据，不能给出最佳区实际边界或敏感性方向。
- 当前点未满足默认 best 准则中的：`Qfus`、`nbar_o_Sudo`。
