...
- 当前运行点：`valid=1`、`ignited=0`；`Pfus=641.7`、`Qfus=1.571`、`Pheat=408.5`、`Pwall=0.6591`。
...
# 2. 关键指标解读
- 功率与损失：`Pfus=641.7`、`Qfus=1.571`、`Pheat=408.5`、`Pwall=0.6591`、`Ptrans=453.8`、`Pbrem=81.55`、`Pcycl=1.581`、`P_line=0`、`Pn=513.4`。
- 约束、成分与几何：`tauE_used=1`、`Zeff=1.867`、`Vp=1149`、`Sw=1593`。
- 仿星器 Stellarator 专属指标：`H_ISS04=0.8617`、`betaT=0.02647`、`nbar_o_Sudo=1.008`、`iota=0.8221`、`aspect_vol=10.01`。
- 当前点对默认 best 准则的逐项核对：`Pfus=641.7` 满足 `>=10`；`Qfus=1.571` 满足 `>=1`；`nbar_o_Sudo=1.008` 不满足 `<=1`；`betaT=0.02647` 满足 `<=0.05`；`H_ISS04=0.8617` 满足 `<=1.5`。

# 3. 风险与不确定性
- `valid` 只描述单点数值有效性，不能替代 POPCON 工作窗证据。
- 扫描缺少完整 `x/y` 坐标证据，不能给出最佳区实际边界或敏感性方向。
- 当前点未满足默认 best 准则中的：`nbar_o_Sudo`。
